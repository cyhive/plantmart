...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    // Reset active photo zoom state when image changes
    setIsZoomed(false);
  }, [activeImage]);
  ...
  return (
    <div className="min-h-screen bg-linear-to-b from-[#fbfdfb] via-[#f7faf7] to-[#f4f7f4] pb-28 relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 mt-8">
          ...
          <div className="lg:col-span-5 lg:sticky lg:top-28 h-fit space-y-6">
            ...
            >
              ...
              {/* Floating Magnify Instruction Overlay */}
              <div className="absolute top-4 right-4 px-3 py-1.5 bg-slate-900/60 backdrop-blur-md rounded-xl text-white text-[9px] font-black uppercase tracking-wider flex items-center gap-1 opacity-100 group-hover:opacity-0 transition-opacity duration-300">
                <Eye className="w-3 h-3 text-emerald-300" /> Hover to Zoom
              </div>
              
              {/* Floating Origin / Certificate Tag */}
              <div className="absolute bottom-6 left-6 px-4 py-2 bg-slate-950/80 backdrop-blur-md rounded-2xl text-white text-[10px] font-black uppercase tracking-wider flex items-center gap-1.5 shadow-lg border border-white/10 pointer-events-none">
                <Leaf className="w-3.5 h-3.5 text-emerald-400 animate-pulse" /> Certified Botanical Specimen
              </div>
            </div>
            
            {/* Gallery Thumbnail Switcher */}
            <div className="flex gap-4 overflow-x-auto pb-2 px-1 no-scrollbar scroll-smooth">
              {product.images.map((img, i) => {
                ...
                return (
                  <button 
                    ...
                    className={`w-20 h-20 md:w-24 md:h-24 rounded-[24px] p-1 flex-shrink-0 transition-all duration-300 cursor-pointer ${
                      isActive 
                        ? 'bg-emerald-600 scale-105 shadow-md shadow-emerald-600/20' 
                        : 'bg-white hover:bg-emerald-55/50 border border-slate-200/50'
                    }`}
                  >
                    <div className="w-full h-full rounded-[18px] overflow-hidden">
                      <img src={img} alt="" className="w-full h-full object-cover transition-transform duration-500 hover:scale-108" />
                    </div>
                  </button>
                );
              })}
          ...
          <div className="lg:col-span-7 space-y-8 lg:pl-4">
            ...
            {/* Config Box: Pot Sizes, Quantity Selectors & Sum Prices */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white p-6 rounded-[32px] border border-slate-100 shadow-[0_12px_30px_rgba(0,0,0,0.02)]">
              
              {/* Pot Size Select */}
              <div className="flex flex-col gap-2">
                <span className="text-[10px] text-slate-400 uppercase font-black tracking-widest ml-1">Pot Size Selection</span>
                <div className="flex items-center bg-slate-50 rounded-2xl p-1 border border-slate-200/50 gap-1.5">
                  {(['Small', 'Medium', 'Large'] as const).map(sz => {
                    const adjust = sizeAdjustments[sz];
                    const isSelected = selectedSize === sz;
                    return (
                      <button
                        key={sz}
                        onClick={() => setSelectedSize(sz)}
                        className={`flex-1 py-2 rounded-xl text-xs font-black transition-all cursor-pointer flex flex-col items-center ${
                          isSelected
                      ...
                      >
                        <span>{sz}</span>
                        <span className={`text-[9px] mt-0.5 font-bold ${isSelected ? 'text-emerald-200' : 'text-slate-400'}`}>
                          {adjust > 0 ? `+₹${adjust}` : 'Base'}
                        </span>
                      </button>
                    );
                  })}
              ...
              {/* Quantity Select & Total Display */}
              <div className="flex items-center justify-between gap-4 border-l-0 md:border-l border-slate-100 pl-0 md:pl-6">
                
                <div className="flex flex-col gap-2">
                  <span className="text-[10px] text-slate-400 uppercase font-black tracking-widest ml-1">Specimen Qty</span>
                  <div className="flex items-center bg-slate-50 rounded-2xl p-1 border border-slate-200/50">
                    <button 
                      onClick={() => setQuantity(q => Math.max(1, q - 1))}
                      disabled={quantity <= 1}
                      className="w-10 h-10 flex items-center justify-center rounded-xl bg-white hover:bg-slate-100 text-slate-600 disabled:opacity-20 disabled:cursor-not-allowed shadow-xs transition-all active:scale-95 cursor-pointer"
                    >
                      <Minus className="w-4 h-4" />
                    </button>
                    <div className="w-10 text-center font-display font-black text-lg text-slate-900 select-none">
                      {quantity}
                    </div>
                    <button 
                  ...
                  <div className="space-y-10">
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between gap-6 pb-6 border-b border-slate-100">
                      ...
                      <button 
                        ...
                        className="w-full md:w-auto text-center text-xs font-black uppercase tracking-widest text-emerald-800 transition-all bg-emerald-50 hover:bg-emerald-700 hover:text-white border border-emerald-200/50 px-8 py-4 rounded-2xl cursor-pointer"
                      >
                        Publish a Review
                      </button>
                    </div>
                    ...
                    <div className="space-y-6">
                      {reviews.map(review => (
                        <div key={review.id} className="p-6 md:p-8 bg-white rounded-[32px] border border-slate-100 shadow-[0_8px_20px_rgba(0,0,0,0.01)] space-y-4">
                          <div className="flex items-start justify-between flex-wrap gap-4">
                            <div className="flex items-center gap-4">
                              <div className="w-12 h-12 bg-emerald-50 rounded-2xl flex items-center justify-center text-emerald-800 font-display font-black text-lg shadow-inner">
                          ...
                          <p className="text-slate-600 font-medium leading-relaxed italic text-sm font-serif">&quot;{review.comment}&quot;</p>
                          
                          <div className="flex items-center gap-4 pt-2 text-xs font-bold">
                            <button 
                              onClick={() => handleHelpfulClick(review.id)}
                              className={`flex items-center gap-1.5 transition-colors cursor-pointer ${
                                review.userClickedHelpful ? 'text-emerald-700 font-extrabold' : 'text-slate-400 hover:text-slate-600'
                              }`}
                            >
                      ...
                      <button className="w-full py-4 text-xs font-black uppercase tracking-widest text-slate-500 hover:text-slate-700 transition-colors border-2 border-slate-100 rounded-2xl hover:border-slate-200 border-dashed cursor-pointer">
                        Load More Reviews
                      </button>
                    </div>
                  </div>
      ...
      {/* Modern High-End Checkout Drawer Preview Modal */}
      <AnimatePresence>
        {showCheckoutPreview && (
      ...
      {/* Universal Floating Quick-Action Bar (Premium bottom-right pill) */}
      <motion.div 
        initial={{ y: 120, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        className="fixed bottom-6 left-1/2 -translate-x-1/2 lg:left-auto lg:right-[calc(max(32px,50vw-640px+32px))] lg:translate-x-0 z-40 w-[92%] sm:w-auto lg:w-auto max-w-4xl bg-white/80 backdrop-blur-2xl border border-white/40 shadow-[0_24px_50px_rgba(0,0,0,0.15)] rounded-[32px] p-3 flex items-center justify-between sm:justify-start gap-4 md:gap-6"
      >
        <div className="hidden sm:flex items-center gap-3 pl-3">
          <div className="w-12 h-12 rounded-2xl overflow-hidden shadow-md bg-white border border-slate-100 flex-shrink-0">
            <img src={product.images[0]} alt={product.name} className="w-full h-full object-cover" />
          </div>
          <div>
            <h4 className="font-display font-black text-sm text-slate-800 leading-tight">{product.name}</h4>
            <p className="text-[10px] font-black uppercase tracking-wider text-emerald-800">{selectedSize} Pot</p>
          </div>
        </div>
        ...
        <div className="pl-3 sm:pl-0">
          <span className="text-[9px] text-slate-400 uppercase font-black tracking-widest block leading-none mb-0.5">Total Price</span>
          <span className="text-xl font-display font-black text-emerald-955">₹{currentPrice * quantity}</span>
        </div>
