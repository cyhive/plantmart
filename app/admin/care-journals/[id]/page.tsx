...
export default function EditCareJournalPage() {
  ...
  useEffect(() => {
    if (id) {
      fetch(`/api/admin/care-journals/${id}`)
        .then(res => res.json())
        .then(data => {
          if (data.journal) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/admin/care-journals/${id}`, {
        method: 'PUT',
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-display font-bold text-slate-900 tracking-tight">Edit Care Journal</h1>
          <p className="text-slate-500 font-medium">Update the plant care guide or article.</p>
        </div>
      </div>
